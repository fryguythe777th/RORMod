...
namespace RiskOfTerrain.Projectiles.Accessory.Damaging
{
    public class FireworkBlast : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            {
                int dust = DustID.Firework_Red;
                switch (Projectile.frame)
                {
                    case 0:
                        dust = DustID.Firework_Red; break;

                    case 1:
                        dust = DustID.Firework_Green; break;

                    case 2:
                        dust = DustID.Firework_Blue; break;

                    case 3:
                        dust = DustID.Firework_Yellow; break;
                }

                for (int i = 0; i < 16 * Projectile.scale; i++)
                {
                    var normal = Main.rand.NextVector2Unit();
                    var d = Dust.NewDustPerfect(Projectile.Center + normal * size * 0.7f * Main.rand.NextFloat(0.5f, 0.8f), dust, normal.RotatedBy(Main.rand.NextFloat(0.1f)) * Main.rand.NextFloat(0.5f, 1f) * 6f, Scale: Main.rand.NextFloat(0.6f, 4f) * Projectile.scale);
                    d.noGravity = true;
                }
            }
